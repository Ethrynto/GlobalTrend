...
class OrderRepository
{
    ...
    {
        return Order::with('customer', 'packages', 'packages.seller', 'packages.delivery_states', 'shipping_address', 'billing_address', 'packages.products', 'packages.products.seller_product_sku.product_variations', 'address')->findOrFail($id);
    }
    ...
    public function orderFindByPaymentDetails($data){
        $order_payment = OrderPayment::where('payment_details', $data)->first();
        if(!$order_payment){
            return $data;
        }
        return Order::with('customer', 'packages', 'packages.seller', 'packages.delivery_states', 'shipping_address', 'billing_address', 'packages.products', 'packages.products.seller_product_sku.product_variations')
            ->where('order_payment_id',$order_payment->id)->first();
    }
    ...
    public function orderStore($data)
    {
        ...
        $order->is_paid = ($data['payment_method'] != 1 && $data['payment_method'] != 7 && $data['payment_method'] != 15) ? 1 : 0;
        ...
        if(isModuleActive('MultiVendor')){
            foreach ($productList as $key => $products) {
                ...
                $package = OrderPackageDetail::create([
                    ...
                    'is_paid' => ($data['payment_method'] != 1 && $data['payment_method'] != 7 && $data['payment_method'] != 15) ? 1 : 0
    ...
    public function orderPaymentDone($amount, $method, $response, $user = null, $order_id = null)
    {
        ...
        if($method != 1 && $method != 2 && $method != 7 && $method != 6){
