...
use Modules\PaymentGateway\Http\Controllers\AmeriaBankController;
use Modules\PaymentGateway\Http\Controllers\IdramController;
...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::rollback();
            Toastr::error($e->getTraceAsString());
            return back();
        }
    }
    ...
    public function payment(Request $request)
    {
        ...
        //return response()->json(['payment_id'=>json_encode($request->method)]);
        ...
        }elseif($request->method == "AmeriaBank"){
            $data['gateway_id'] = encrypt(15);
            $ameriaBankController = new AmeriaBankController();
            $response = $ameriaBankController->payment(array_merge($request->all(), ['OrderID' => $this->generateOrderID()]));
            // return response()->json(['payment_id'=>json_encode($response)]);
            //return Redirect::to("https://servicestest.ameriabank.am/VPOS/Payments/Pay?id=".$response["PaymentID"]."&lang=en");
    ...
    public function order_summary(Request $request, $id=null)
    {
        try {
            if ($request->has('orderID'))
            {
                $ameriaBankController = new AmeriaBankController();
                $credentials = $ameriaBankController->getCredential();
                $username = @$credentials->perameter_2;
                $password = @$credentials->perameter_3;
                $data['order'] = $this->orderService->orderFindByPaymentDetails($request->orderID);
                //LogActivity::successLog($data['order']);
                // return response()->json($data);
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, "https://services.ameriabank.am/VPOS/api/VPOS/GetPaymentDetails");
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
                    "PaymentID" => $request->get('orderID'),
                    "Username" => $username,
                    "Password" => $password,
                ...
                // LogActivity::successLog($response);
                curl_close($ch);
                // return response()->json($response);
                if($data['order']['is_paid'] == 0 && $request->get('resposneCode') == "00"){
                    Toastr::success($request->get('description'));
                    Order::where('order_number',$data['order']['order_number'])->update(['is_paid' => 1]);
                }else{
                    Toastr::error($request->get('description'));
                }
