...
<head>
    ...
    <link rel="stylesheet" href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/dist/css/keditor.css"
          data-type="keditor-style"/>
    <link rel="stylesheet"
          href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/dist/css/keditor-components.css"
          data-type="keditor-style"/>
    <!-- End of KEditor styles -->
    <link rel="stylesheet"
          href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/plugins/code-prettify/src/prettify.css"/>
    <link rel="stylesheet" href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/css/examples.css"/>
    <link rel="stylesheet" data-type="keditor-style"
          href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/css/style.css"/>
    @php
        $themeColor = Modules\Appearance\Entities\ThemeColor::where('status',1)->first();
        $LanguageList = getLanguageList();
        $languageSelector = '<select id="language-selector" class="keditor-ui">';
        foreach ($LanguageList as $key => $language){
            $languageSelector .= '<option value="'.$language->code.'" '.(auth()->user()->lang_code == $language->code ? "selected" : "").'>'.mb_strtoupper($language->code).'</option>';
        }
    ...
    <link rel="stylesheet" data-type="keditor-style"
          href="{{asset('Modules/PageBuilder/Resources/assets/css/affiliate.css')}}">
    <link rel="stylesheet"
          href="{{asset('Modules/PageBuilder/Resources/assets')}}/keditor/plugins/font-awesome-4.7.0/css/font-awesome.min.css"
          data-type="keditor-style"/>
    <style>
        ...
        select#language-selector:not(:hover) {
            background: #1b1d27;
        }
    </style>
</head>
...
<div style="display: none">
    @foreach ($LanguageList as $key => $language)
        <div id="lang-html-{{$language->code}}">
            {{isset($row)?$row->getTranslation('description',$language->code):old('description.'.$language->code)}}
        </div>
    @endforeach
</div>
<div data-keditor="html">
    <div id="content-area">
        {!! $row->getTranslation('description',auth()->user()->lang_code) !!}
    </div>
</div>
...
<script data-keditor="script">
    $(function () {
        const keditor = $('#content-area').keditor({
            ...
            onSave: function (content) {
                ...
                let data = {};
                let local = $('#language-selector').val();
                data[local] = content
                $.ajax({
                    url: url,
                    type: "PUT",
        ...
        $('body').on('change', '#language-selector', function () {
            let selectedLanguage = $(this).val();
            let content = $(`#lang-html-${selectedLanguage}`).text();
            $('#content-area').text(content);
            keditor.setContent('', '.keditor-container-content-inner');
            if (content.trim() !== '') {
                keditor.setContent(content);
            }
        });
    });
