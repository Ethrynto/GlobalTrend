...
class OrderManageController extends Controller
{
    ...
    public function ameriaRefund(Request $request){
        $request->validate([
            'payment_id' => 'required',
            'amount' => 'required',
        ]);
        $ameria = new AmeriaBankController();
        $response = $ameria->refund($request->payment_id, $request->amount);
        return $response;

    }
    public function ameriaCancel(Request $request){
        $request->validate([
            'payment_id' => 'required',
        ]);
        $ameria = new AmeriaBankController();
        $response = $ameria->cancel($request->payment_id);
        return $response;
    }
}
