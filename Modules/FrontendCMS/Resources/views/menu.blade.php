...
<li class="{{ $frontend_cms ?'mm-active' : '' }} sortable_li" data-position="{{ menuManagerCheck(1,2)->position }}" data-status="{{ menuManagerCheck(1,2)->status }}">
    ...
    <ul id="frontend_cms">
        ...
        @endif
        @if (permissionCheck('frontendcms.socialLink') && menuManagerCheck(2,2,'frontendcms.socialLink')->status == 1 && app('theme')->folder_path == 'amazy')
            <li data-position="{{ menuManagerCheck(2,2,'frontendcms.socialLink')->position }}">
                <a href="{{ route('frontendcms.socialLink') }}" class="{{request()->is('frontendcms/socialLink') ? 'active' : ''}}">{{ __('frontendCms.socialLink') }}</a>
            </li>
        @endif
    </ul>
</li>
