            ...
            <form action="{{ route('register') }}" method="POST" name="register" id="register_form" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    @else
                        ...
                        @if(!isset($_SESSION)) <?php session_start();?> @endif
                        <div class="col-12 mb_20">
                            <label for="referral_code" class="primary_label2">{{__('common.referral_code_(optional)')}}</label>
                            <input name="referral_code" id="referral_code" value="<?php if(isset($_SESSION['source_code'])) echo $_SESSION['source_code']; else echo old('referral_code') ?>" placeholder="{{ __('common.referral_code') }}" onfocus="this.placeholder = ''" onblur="this.placeholder = '{{ __('common.referral_code') }}'" class="primary_input3 radius_5px" type="text">
