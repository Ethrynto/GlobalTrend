...
<form id=idram action="https://money.idram.am/payment.aspx" method="POST">
<input type="hidden" name="EDP_LANGUAGE" value="EN">
<input type="hidden" name="EDP_REC_ACCOUNT" value="100047474">
<input type="hidden" name="EDP_DESCRIPTION" value="Order {{ getNumberTranslate($order->order_number) }}">
<input type="hidden" name="EDP_AMOUNT" value="{{ explode(' ',single_price($order->grand_total))[1] }}">
<input type="hidden" name="EDP_BILL_NO" value="{{ getNumberTranslate($order->order_number) }}">
</form>
<script>
var results = document.cookie.match(/paysys=(.+?)(;|$)/);

if(results[1]==6){
document.getElementById("idram").submit();
document.cookie="paysys=;path=/;"
}
if(results[1]==15){
    window.location.href = "https://services.ameriabank.am/VPOS/Payments/Pay?id={{$order->order_payment->txn_id}}&lang=en";
    document.cookie="paysys=;path=/;"
}
</script>
...
if(isset($_GET['op'])){
if($_GET['op']!='fail'){
?>{{ __('defaultTheme.thank_you_for_your_purchase') }}!<?php
}
else {
?>
Payment Fail! Your order is done but not paid.
<?php
}
}
else {?>
{{ __('defaultTheme.thank_you_for_your_purchase') }}!
<?php
}
?>
</h3>
